...
void GetCityData()
{
	...
	{
		sprintf(city_name[iCounter],"%c",0);
...
//	_strdate(cdate);
	...
	fprintf(stream,"%.30s\r\n%s\r\n%u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u ",plr_name,cdate,plr_money,plr_taxpaid,
		...
		plr_advdecisions[2],plr_advdecisions[3]);
...
bool LoadGame()
{
	...
	fscanf(stream,"%u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u ",&plr_money,&ltemp,
		&plr_crew[0],&plr_crew[1],&plr_crew[2],&plr_crew[3],&plr_reputation,&plr_careful,&plr_gametype,&plr_currentcity,
		&plr_currentcustomer,&plr_jobsdone,&plr_daysinbusiness,&plr_lastspecial,&plr_advdecisions[0],&plr_advdecisions[1],
		&plr_advdecisions[2],&plr_advdecisions[3]);
	plr_taxpaid=ltemp;
...
//printf("LOAD lcheck %u lcheck2 %u\n",lcheck,lcheck2);
	...
	// HERE COMES THE CHEAT DETECTION, FIX THIS TO FINAL VERSION!!!
...
//	char cBFTemp[100];
	...
	{
	fscanf(stream,"%u %u %u %u %u %u %u %u %u ",&SIZE_X,&SIZE_Y,&setBits,&setSoundEffects,&setExplosions,&setDustClouds, &setScroll, &setMaxFramerate, &iTmp);
...
void SaveSettings()
{
	...
	{
	fprintf(stream,"%u %u %u %u %u %u %u %u %s\n%s",SIZE_X,SIZE_Y,setBits,setSoundEffects,setExplosions,setDustClouds,setScroll,setMaxFramerate, cButtonFile,cLangfile);
	CloseFile();
	}
}
...
void GetHiscores()
{
	...
	while(!feof(stream) && iCounter<MAXHISCORE)
	{
		fgets(tmp,35,stream);
...
//		hs_player[iCounter][strlen(hs_player[iCounter])-2]=0;
		...
		fscanf(stream,"%u %u %d %u ",&hs_money[iCounter],&hs_gametype[iCounter],&hs_reputation[iCounter],&hs_daysinbusiness[iCounter]);
...
//	for(iCounter=0;iCounter<12;iCounter++) printf("hs_player[%d]=%s %s %u %u %u \n",iCounter, hs_player[iCounter],hs_date[iCounter],hs_money[iCounter],hs_reputation[iCounter],hs_daysinbusiness[iCounter]);
	...
	if(lCheck!=lCheck2)	// fix this to final version, hiscore list cheat prevention!!!
	{
//		printf("Hiscore check failed %u %u\n",lCheck,lCheck2);
		for(il=0;il<MAXHISCORE;il++)
		{
...
void SaveHiscores()
{
	...
	for(iCounter=0;iCounter<MAXHISCORE;iCounter++)
	{
		...
		fprintf(stream,"%s\r\n%s\r\n%u %u %d %u\r\n",hs_player[iCounter],hs_date[iCounter],hs_money[iCounter],hs_gametype[iCounter],hs_reputation[iCounter],hs_daysinbusiness[iCounter]);
	}
...
{// illegal chars: \/:*?"<>|
	...
	if(OpenFile("maptemp.tmp","wb",0)==0)
	{
		...
		fprintf(stream," %u %u %u ", p_buildbegin, p_buildwidth,ed_currentcity);
...
short LoadMap(char *s, short iCode)	// icode 0=no zunpack
{
	short ix,iy;
	char *s2="";
...
//		p_buildwidth=ix-p_buildbegin;
		...
		for(ix=0;ix<10;ix++)
		{
			...
			temp_ed_message[ix][strlen(temp_ed_message[ix])-1]=0;
		}
