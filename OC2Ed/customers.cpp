...
signed short FindFirstCustomer()
{
	...
	if(map_city[ix]>=0)
	{
		if(map_city[ix]==plr_currentcity && plr_usedmaps[ix]==0 && iresult==-1) iresult=ix;
	}
...
bool CheckCareerCustomers()
{
	...
	if(imatch>=196)		// current nr of maps, change if amount changes!!!
		return true;
	else
		return false;
}
...
//	if(boolcustomercounted==false) GenerateCustomerData();
	...
	if(ed_customer[0]!=0) sprintf(msg,"%s",ed_customer);	// version 1.1
	...
	if(map_nrc[sel_customer][1]==58)	// :		version 1.1 begins
		...
		if(bOk==true)
		{
//		DrawNormalText(gametxt[],xmiddle-240,ymiddle-200,iTextColor);	// map file 
		for(ix=2;ix<52;ix++)
		if(map_nrc[sel_customer][ix]>31)
			fn[ix-2]=map_nrc[sel_customer][ix];
		sprintf(msg,"%s",fn);
...
//	sprintf(msg,"%-s",b_name[customer_building]);
	...
	if(iy==1) DrawNormalText("Messages",xmiddle-240,ymiddle-140,iTextColor);	// messages 
