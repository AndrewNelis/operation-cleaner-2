...
extern long				lTimer,lPlacehours,lExtraCosts;
long					lLastSound=0;
...
void HandleBlocks()
{
	...
	short ix,iy,side,addx,addy,idst;
//	short iymin[MAPSIZE];
	bool bdone=false;
	bool badd; //,bmove;
	double dTempdam;
...
//	do
	{
		...
		for(ix=1;ix<MAPSIZE-1;ix++)
			...
			{
//				if(map[ix][iy]>0 && map[ix][iy]==0 && map[ix][iy]==0
/*				if(mapv[ix][iy]==0)
				{
					if(mapvertv[ix][iy]>0) mapvertv[ix][iy]--;
					if(mapvertv[ix][iy]<0) mapvertv[ix][iy]++;
				}
*/
				if(map[ix][iy]>0 && mapcounted[ix][iy]==0 && mapv[ix][iy]>0)	//mapsupport[ix][iy]<0
				{
...
//					bmove=false;
					...
					if(mapv[ix][iy]>0) // || mapvertv[ix][iy]!=0)
/*					{
						bmove=true;
					}
...
//							if(mapvertv[ix][iy]<0) addx=-1;
						...
						if(map[ix+addx][iy+addy]>0)
							{
							ReduceEnd(ix,iy+addy,short(dTempdam));
							if(map[ix+addx][iy]>0) ReduceEnd(ix+addx,iy,short(dTempdam));
...
//							mapvertv[ix+addx][iy+addy]=mapvertv[ix][iy];
							...
							mapcounted[ix+addx][iy+addy]=mapcounted[ix][iy];
...
//					mapvertv[ix][iy]=0;
	...
	if(blocksmoving==true)
	{
		...
		for(ix=0;ix<MAPSIZE;ix++) {mapv[ix][1]=0;mapvertv[ix][1]=0;}
	}
...
void ZeroMap(short iCode)	// code not used in oc2ed
{
	short xc,yc;
...
//	BeginRandomization();
	...
	sprintf(ed_building,"\0");
	for(xc=0;xc<10;xc++) sprintf(ed_message[xc],"");
	ed_currentcity=0;
	sprintf(ed_customer,"\0");
...
sprintf(ed_filename,"%s %u",gametxt[28],randseed);
for(ix=0;ix<(short) strlen(ed_filename);ix++)
	if(ed_filename[ix]==32) ed_filename[ix]=95;
...
//	sprintf(msg,"T:%u O:%u",lTargetValue,lOtherValue);	// REMOVE FROM FINAL
//	AddMessage(msg,1);
