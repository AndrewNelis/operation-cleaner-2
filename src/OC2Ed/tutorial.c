...
void DrawTutorial()
{
	...
	if(cTutorial<6)
	{
		if(GameOn!=3 && GameOn!=5) DrawBMPButton("",ihlpx[cTutorial],ihlpy[cTutorial],12);
	}
...
//		printf("%s %d\n",gametxt[i],iTextColor);
	...
	if(cTutorial==7)
	{
		...
		if(plr_money<7000) strcpy(cOKBoxText[0],gametxt[490]);						//You'll need some more practise.
		if(plr_money>=7000 && plr_money<=11000) strcpy(cOKBoxText[0],gametxt[491]);	//That was a pretty decent detonation.
		if(plr_money>=11000) strcpy(cOKBoxText[0],gametxt[492]);					//Excellent job!
