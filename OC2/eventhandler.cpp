...
if(bInputBox==true)
{
	...
	if(code==8 && cInputCursor>0)	// backspace
	{
		cInputCursor--;
...

	{
        ...
        switch (event.type) 
		{
            ...
            case SDL_MOUSEBUTTONDOWN: 
				...
				m_button=event.button.button;		// 1 LMB, 2 RMB, 3 MMB
				m_button_up=0;
				m_event=1;
...
//						bAddRandseed=false;	
				...
				if(keys[SDLK_KP_PLUS]==SDL_PRESSED || keys[SDLK_PLUS]==SDL_PRESSED )
					if(ec_amount[ec_selected]<25) ec_amount[ec_selected]++;

				if(keys[SDLK_KP_MINUS]==SDL_PRESSED || keys[SDLK_MINUS]==SDL_PRESSED)
					if(ec_amount[ec_selected]>1) ec_amount[ec_selected]--;

				if(keys[SDLK_PAGEUP]==SDL_PRESSED)
					if(ec_timer[ec_selected]<250) ec_timer[ec_selected]++;

				if(keys[SDLK_PAGEDOWN]==SDL_PRESSED)
					if(ec_timer[ec_selected]>0) ec_timer[ec_selected]--;
...
/*				if(keys[SDLK_a]==SDL_PRESSED)	// add days and money
				{
					plr_money+=(rand()%100+10)*100;
//					plr_money=600000;
					AddDays(25);
				}
*/
